...
import cn.xls.icf.flowable.repository.model.DeploymentVO;
import cn.xls.icf.flowable.service.inf.IProcessService;
...
public class ProcessController {
    ...
    public ResponseData deployByZip(String name, String category, String tenantId, MultipartFile file) {
        ...
        try (ZipInputStream zipIn = new ZipInputStream(file.getInputStream(), Charset.forName("UTF-8"))) {
            deployment = processService.deploy(name, category, tenantId, zipIn);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public ResponseData<DeploymentVO> deployByInputStream(String name, String tenantId, String category, MultipartFile file) {
        ...
        if (processService.exist(name)) {
            deploy = processService.deployName(name);
        } else {
            try (InputStream in = file.getInputStream()) {
                deploy = processService.deploy(name, tenantId, category, in);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
